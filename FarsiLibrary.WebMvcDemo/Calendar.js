	...
	return {
		...
		**/
		...
		draw: function()
		{
			...
			_openCal = jQuery(this).parent().find('>div.popup-calendar-wrapper');
            _openCal.append(_getCalendarDiv());
		}		
	};
}();
...
jQuery.fn.datePicker = function(a)
{
	this.each(function() {
		...
		if (!jQuery.datePicker.isInited(this)) 
		{
			...
			{
				calBut = jQuery("<a></a>")
				            .attr({'href':'javascript:;', 'class':'date-picker', 'title':chooseDate})
				            .append("<span>" + chooseDate + "</span>");
			}
			
			jQuery(this).wrap('<div class="date-picker-holder"></div>')
			            .after(jQuery('<div></div>')
        				    .attr('class', 'popup-calendar-wrapper')
		        		    .append(jQuery("<div></div>").attr({'class':'popup-calendar'})
				        ),	calBut);
...
jQuery.fn.Calendar = function(a)
{
    this.each(function() 
    {
		jQuery(this).append().attr({'class':'popup-calendar-wrapper'})
		            .append(jQuery("<div></div>").attr({'class':'popup-calendar'}));

    
        jQuery.datePicker.draw();
    });
};